...
{
    constexpr RakNet::TimeMS milliseconds = 1 * 15 * 1000;
    constexpr RakNet::TimeMS millisecondsPerUpdate = 10;
    constexpr size_t actionsPerUpdate = NUM_PEERS * 5;
    ...
    RakNet::TimeMS nextUpdate = now;
    RakNet::TimeMS endTime = now + milliseconds;
	while (static_cast<int32_t>(endTime - now) > 0)
	{
        for (size_t i = 0; i < actionsPerUpdate; ++i)
        {
    ...
    //			dataLength=600+(randomMT()%7000);
            ...
            {
                while (auto* p = env.peers[i]->Receive())
                {
                    env.peers[i]->DeallocatePacket(p);
                }
            }
        }

        nextUpdate += millisecondsPerUpdate;
        now = RakNet::GetTimeMS();
        int32_t delta = static_cast<int32_t>(nextUpdate - now);
