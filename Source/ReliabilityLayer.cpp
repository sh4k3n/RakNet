...
namespace RakNet
{
    ...
    inline void EnsureStream(struct IKCPCB*& ikcp, uint32_t channel, RemoteSystem& remoteSystem)
    {
        if (ikcp == nullptr)
        {
            ikcp = ikcp_create(channel, &remoteSystem);
            ikcp_nodelay(ikcp, 1, 20, 2, 0);
            ikcp->output = SendKCPPacket;
            //ikcp_update(ikcp, currentTime);
        }
    }
}
#endif
...
BPSTracker::TimeAndValue2::~TimeAndValue2() {}
BPSTracker::TimeAndValue2::TimeAndValue2(CCTimeType t, uint64_t v1) : value1(v1), time(t) {}
...
void ReliabilityLayer::Reset(bool resetVariables, int MTUSize, bool _useSecurity )
{
    ...
    {
        if (myOrderedStreams[i])
        {
            ikcp_flush(myOrderedStreams[i]);
        }
    }    
#else
	...
	if (resetVariables)
	{
        ...
        for (size_t i = 0; i < NUMBER_OF_ORDERED_STREAMS; ++i)
        {
            if (myOrderedStreams[i])
            {
                ikcp_release(myOrderedStreams[i]);
                myOrderedStreams[i] = nullptr;
            }
        }
#else
...
bool ReliabilityLayer::HandleSocketReceiveFromConnectedPlayer(
    ...
    RakNetRandom *rnr, CCTimeType timeRead,
	BitStream &updateBitStream)
{
    ...
    {
        EnsureStream(myOrderedStreams[orderingChannel], orderingChannel, remoteSystem);
        if (ikcp_input(myOrderedStreams[orderingChannel], buffer, long(length)) >= 0)
        {
            return true;
        }
    }
...
// 					printf("waiting on reliableMessageNumber=%i holeCount=%i datagramNumber=%i\n", receivedPacketsBaseIndex.val, holeCount.val, dhf.datagramNumber.val);
				...
				if ( internalPacket->splitPacketCount > 0 )
				{
					...
					internalPacket = BuildPacketFromSplitPacketList(internalPacket->splitPacketId, timeRead,
                        remoteSystem, rnr, updateBitStream);
...
BitSize_t ReliabilityLayer::Receive( unsigned char **data )
{
    ...
    do 
    {
        if (myOrderedStreams[myNextReceiveIndex])
        {
            int nextSize = ikcp_peeksize(myOrderedStreams[myNextReceiveIndex]);
            if (nextSize > 0)
            {
                char* newBuffer = (char*)malloc(nextSize);
                auto res = ikcp_recv(myOrderedStreams[myNextReceiveIndex], newBuffer, nextSize);
                RakAssert(res >= 0);
        ...
        ++myNextReceiveIndex;
    } while (myNextReceiveIndex < NUMBER_OF_ORDERED_STREAMS);
    myNextReceiveIndex = 0;
    return 0;
#else
	InternalPacket * internalPacket;
...
bool ReliabilityLayer::Send( 
    RemoteSystem& remoteSystem,
    char *data, BitSize_t numberOfBitsToSend, PacketPriority priority, 
...
)
{
    ...
    {
        EnsureStream(myOrderedStreams[orderingChannel], orderingChannel, remoteSystem);
        int result = ikcp_send(myOrderedStreams[orderingChannel], data, numberOfBytesToSend);
        if (result >= 0)
        {
...
//-------------------------------------------------------------------------------------------------------
void ReliabilityLayer::Update( RemoteSystem& remoteSystem, int MTUSize, CCTimeType time,
...

{
    ...
    for (size_t i = 0; i < NUMBER_OF_ORDERED_STREAMS; ++i)
    {
        if (myOrderedStreams[i])
        {
            ikcp_update(myOrderedStreams[i], time);
        }
    }
#else
...
#pragma warning( disable : 4127 ) // warning C4127: conditional expression is constant
	...
	{
		SendACKs(remoteSystem, time, rnr, updateBitStream);
	}
	...
	if (NAKs.Size()>0)
	{
		...
		SendBitStream( remoteSystem, &updateBitStream, rnr, time );
	}
...
// 							printf("RESEND reliableMessageNumber %i with datagram %i\n", internalPacket->reliableMessageNumber.val, congestionManager.GetNextDatagramSequenceNumber().val);
						...
						for (unsigned int messageHandlerIndex=0; messageHandlerIndex < messageHandlerList.Size(); messageHandlerIndex++)
						{
							...
							messageHandlerList[messageHandlerIndex]->OnInternalPacket(internalPacket, 
                                packetsToSendThisUpdateDatagramBoundaries.Size()+congestionManager.GetNextDatagramSequenceNumber(), 
                                remoteSystem.systemAddress, (RakNet::TimeMS)(time/(CCTimeType)1000), true);
#endif
						}
...
// 						printf("SEND reliableMessageNumber %i in datagram %i\n", internalPacket->reliableMessageNumber.val, congestionManager.GetNextDatagramSequenceNumber().val);
		...
		for (unsigned int datagramIndex=0; datagramIndex < packetsToSendThisUpdateDatagramBoundaries.Size(); datagramIndex++)
		{
			...
			if (isSecondOfPacketPair)
			{
				...
				RakAssert(updateBitStream.GetNumberOfBytesUsed()<=MAXIMUM_MTU_SIZE-UDP_HEADER_SIZE);
                while (updateBitStream.GetNumberOfBytesUsed() < datagramSizesInBytes[datagramIndex - 1])
                {
                    updateBitStream.Write(uint8_t(0));
                }
				RakAssert(updateBitStream.GetNumberOfBytesUsed()<=MAXIMUM_MTU_SIZE-UDP_HEADER_SIZE);
			}
...
//-------------------------------------------------------------------------------------------------------
void ReliabilityLayer::SendBitStream( RemoteSystem& remoteSystem, RakNet::BitStream *bitStream, RakNetRandom *rnr, CCTimeType currentTime)
{
	(void) rnr;
    ...
    bpsMetrics[(int)ACTUAL_BYTES_SENT].Push1(currentTime, length + UDP_HEADER_SIZE);
...
bool ReliabilityLayer::IsOutgoingDataWaiting(void)
{
    ...
    for (size_t i = 0; i < NUMBER_OF_ORDERED_STREAMS; ++i)
    {
        if (myOrderedStreams[i])
        {
            if (ikcp_waitsnd(myOrderedStreams[i]))
            {
                return true;
            }
        }
...
//-------------------------------------------------------------------------------------------------------
InternalPacket * ReliabilityLayer::BuildPacketFromSplitPacketList( SplitPacketIdType _splitPacketId, CCTimeType time,
																  RemoteSystem& remoteSystem, RakNetRandom *rnr, 
																  BitStream &updateBitStream)
{
	unsigned int i;
	bool objectExists;
	...
	i=splitPacketChannelList.GetIndexFromKey(_splitPacketId, &objectExists);
...
//-------------------------------------------------------------------------------------------------------
void ReliabilityLayer::SendACKs(RemoteSystem& remoteSystem, CCTimeType time, RakNetRandom *rnr, BitStream &updateBitStream)
{
